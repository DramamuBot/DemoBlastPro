...
class AntiSpamGuard:
    ...
    def is_clean_email(cls, email: str) -> bool:
        ...
        # 0. Cek panjang email (RFC 5321: max 254 karakter)
        if len(email) > 254:
            raise SecurityViolation("Email terlalu panjang. Maksimal 254 karakter.")
        # Cek panjang bagian lokal (sebelum @) max 64 karakter (RFC 5321)
        parts = email.split('@')
        if len(parts) == 2 and len(parts[0]) > 64:
            raise SecurityViolation("Bagian lokal email terlalu panjang.")
